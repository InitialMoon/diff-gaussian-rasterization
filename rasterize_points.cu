...
std::function<char*(size_t N)> resizeFunctional(torch::Tensor& t) {
    auto lambda = [&t](size_t N) {
		...
		// contiguous() 方法可以将一个在内存中不连续存储的张量转换为连续存储。
    };
    return lambda;
}
...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& means3D, // 高斯核中心
    const torch::Tensor& colors, // 高斯颜色
    const torch::Tensor& opacity, // 透明度
	const torch::Tensor& scales, // 半径
	const torch::Tensor& rotations, // 旋转矩阵？
	const float scale_modifier, // 尺寸调整器
	const torch::Tensor& cov3D_precomp, // 3d协协方差矩阵
	const torch::Tensor& viewmatrix, // 视图矩阵
	const torch::Tensor& projmatrix, // 投影矩阵
	const float tan_fovx,  // x方向视角大小
	const float tan_fovy, // y方向视角大小
    const int image_height, // 图片高度
    const int image_width, // 图片宽度
	const torch::Tensor& sh, // 球协函数参数
	const int degree, // 
	const torch::Tensor& campos, // 相机位姿？
	const bool prefiltered, // 预处理滤波
	const bool debug) // debug
{
  ...
  const int H = image_height; // 图片高度
  const int W = image_width; // 图片宽度
...
>>>>>>> origin/my_depth
  ...
  // 下面3个函数是调用上面的模板，定义了3个将自己类型数据变为连续存储的张量并返回其指针的函数
  std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
  std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
  std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
  
  int rendered = 0; // 初始化渲染状态
  if(P != 0)
  {
	  int M = 0;
	  if(sh.size(0) != 0)
